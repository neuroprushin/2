"""Workspace manager module for handling file operations and workspace structure."""
# pylama:ignore=E251,E501,E125,C901
...
class BM25Search:
    ...
    def search(self,
        ...
        for path, doc in self.documents.items():
            score = 0.0
            doc_len_norm = 1 - self.b + self.b * (doc.length /
                                                  self.avg_doc_length)
...
class WorkspaceManager:
    ...
    def _is_cache_valid(
            self, path: str,
            cache_entry: Tuple[Union[str, List[dict]], float]) -> bool:
                    ...
                    for entry in it:
                        try:
                            ...
                            if entry.name.startswith(".") or (
                                ...
                                # Debug log
                                print(
                                    f"Skipping {entry.name} (hidden/ignored)")
                                continue
