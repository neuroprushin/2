...
class CodeAnalyzer:
    ...
    def analyze_directory(self, directory: str, file_pattern: str = None) -> List[CodeIssue]:
        ...
        for root, _, files in os.walk(directory):
            for file in files:
                ...
                try:
                    ...
                    if file.endswith('.py'):
                        issues.extend(self._analyze_python_file(relative_path, content))
                    elif file.endswith('.js'):
                        issues.extend(self._analyze_javascript_file(relative_path, content))
                    else:
                        issues.extend(self._analyze_generic_file(relative_path, content))
    ...
    def _is_code_file(self, filename: str) -> bool:
        ...
        code_extensions = {'.py', '.js', '.jsx', '.ts', '.tsx', '.java', '.cpp', '.c', '.h', '.cs', '.php', '.rb', '.go'}
        return any(filename.endswith(ext) for ext in code_extensions)
    
    def _analyze_python_file(self, file_path: str, content: str) -> List[CodeIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.match(r'^\s*def\s+\w+\s*\(', line):
                ...
                if params:
                    param_count = len([p for p in params.group(1).split(',') if p.strip()])
                    if param_count > self.max_params:
                        issues.append(CodeIssue(
                            ...
                            suggestion="Consider grouping parameters into a class or using keyword arguments"
                        ))
            ...
            if in_function:
                ...
                if function_lines > self.max_function_length:
                    issues.append(CodeIssue(
                        ...
                        line=i,
                        issue_type="complexity",
                        message=f"Function is {function_lines} lines long (max {self.max_function_length})",
    ...
    def _analyze_javascript_file(self, file_path: str, content: str) -> List[CodeIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Check for function definitions
            if re.search(r'(function\s+\w+\s*\(|=>|\w+\s*=\s*function\s*\()', line):
                in_function = True
                function_lines = 0
                ...
                if params:
                    ...
                    if param_count > self.max_params:
                        issues.append(CodeIssue(
                            ...
                            issue_type="complexity",
                            message=f"Function has {param_count} parameters (max {self.max_params})",
                            suggestion="Consider grouping parameters into an object or using destructuring"
                        ))
    ...
    def _analyze_generic_file(self, file_path: str, content: str) -> List[CodeIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r'[\'"]\d+[\'"]|\b\d{4,}\b', line):
                issues.append(CodeIssue(
                    ...
                    suggestion="Consider using a named constant or configuration value"
                ))
